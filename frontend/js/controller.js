$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: "../../backend/serviceHandler.php",  // Update the URL to point to your PHP file
        cache: false,
...
/*$(document).ready(function () {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            // Iterate through the response array and append appointment data to each accordion body
            response.forEach(function (appointmentArray, index) {
                var appointment = appointmentArray[0];  // Get the Appointment object from the array

                var appointmentHtml = '<div class="card">' +
                    '<div class="card-body">' +
                    '<h5 class="card-title">' + appointment.title + '</h5>' +
                    '<h6 class="card-subtitle mb-2 text-muted">' + appointment.date + '</h6>' +
                    '<p class="card-text">Name: ' + appointment.name + '</p>' +
                ...
                // Append the appointment HTML to the corresponding accordion body
                $("#collapse" + (index + 1) + " .accordion-body").append(appointmentHtml);
            });
        },
        error: function (xhr, status, error) {
            console.error("AJAX request failed:", status, error);
